...
%The pattern matching operation is initialized in Unicon by using the \texttt{??} operator. \cite{Gaikaiwari2005} The following expression results in the \texttt{subject} having a pattern match performed with \texttt{pattern}:
%\begin{verbatim}
%        subject ?? pattern
%\end{verbatim}
...
In the string scanning environment each expression is executed individually if it succeeds, the index or cursor location is adjusted.  Therefore if a pattern match is called using the tabmat \texttt{=} operator, the cursor location is adjusted if it succeeds.

\textbf{\subsection{SNOBOL4 and Unicon pattern operators}}
The pattern operators for Unicon were defined by Gaikaiwari in his Master's Thesis.  Although they are lexically different, they are functionally identical to SNOBOL4 pattern operators.  The pattern concatenation operator is similar to the string concatenation operator in that it has the end result of the first string being followed by the second strings.  But, in the case of string concatenation, the two strings are known, while with the pattern concatenation the two strings will not be determined until the program is run.  Therefore pattern concatenation is equivalent to saying this pattern followed by this other pattern.  Conceptually, string concatenation is concrete while pattern concatenation is more abstract as the operands are not yet resolved.  
...
\item Modify the unevaluated expression operand in the pattern matching operation to handle function calls as a parameter.
...
\textbf{\subsection{Pattern matching statements}}
The pattern matching operation as implemented by Gaikaiwari was set be performed in the non-anchored mode by default.  Although the anchored pattern matching operation was defined in the pattern resources as a part of the internal match function, but the \texttt{Anchored\_Mode} identifier was set to false.  The default location of the index was set to 1.  The arguments for the \texttt{internal match} operation were changed to include both the mode and cursor location.  This allows the \texttt{internal match} to be called and initiated from any location of the subject string.

\textbf{\subsubsection{Anchored mode from string scanning}}
It was decided that integrating the pattern matching system into the Unicon string scanning environment would require the pattern matching be performed in the anchored mode, since the string scanning functions are dependent upon the index or cursor position.  For this implementation the Unicon tabmat operator \texttt{=} was determined to be an ideal choice for initiating a pattern match in a string scanning environment.  The use of an equals \texttt{=} before a pattern variable triggers the anchored mode pattern matching operation.
...
\textbf{\subsection{Decomposing phone numbers}}
...
\begin{table}[ht]
	...
	\end{tabular}
\end{table}\\
In this first benchmark the string scanning solution used more lines of code, but used fewer words and characters to accomplish the same result as the other solutions.  Among the various pattern examples the regular pattern and the unevaluated expression examples are the most readable due to the revisions made for lexical consistency.

In \cite{Gaikaiwari2005}, Gaikaiwari used a similar benchmark  and found the pattern solution to perform the task faster.  Although this thesis does not test for speed, another advantage of using patterns may be in its processing speed. 

\textbf{\subsection{Detecting words with double letters}}
...
\begin{table}[ht]
	...
	\end{tabular}
\end{table}
The string scanning example may have used more lines than the pattern examples, but required fewer words or characters than all the other solutions.  Of the pattern examples, the \texttt{||} concatenation operator is more consistent lexically than the \texttt{\&\&} concatenation operator for the Unicon language.

\textbf{\subsection{Number of times a word is used}}
To count the number of times a word is used in a block of text you need the algorithm to count every time the word is used, but not when the word is a part of some other word.  When counting the number of times the word \texttt{the} is used, counting every \texttt{they, them, there} and so forth would not get the desired result.  So the algorithm must be refined enough to recognize the difference.
...
The pattern examples for each of the benchmark problems resulted in the shortest most concise code for the problem.  Patterns did not produce significantly improved code lengths as compared to Gaikaiwari's patterns, but it still produced a significant improvement over the string scanning environment of Unicon in two of the four benchmark problems.

Although pattern matching does not always result in shorter and therefore simpler code, pattern matching is a useful improvement to the Unicon language.  It allows those users that prefer the functionality of SNOBOL4 patterns to use a similar system of string analysis.

The lexical changes to Gaikaiwari's pattern matching is an improvement, as it is more consistent with the Unicon language.  
...
Patterns can handle procedure calls, but the values they are evaluated at pattern construction time.  Therefore it is necessary to use the unevaluated expression to evaluate them at pattern evaluation time if any of the parameters are assigned in the pattern itself.  As it stands now, the unevaluated expressions do not handle procedure calls.
